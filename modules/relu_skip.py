...
class INR(nn.Module):
    def __init__(self, in_features,
        ...
        for i in range(hidden_layers):
            if i == (hidden_layers//2)-1:
                self.net.append(self.nonlin(hidden_features + hidden_features, hidden_features ,  
    ...
    def forward(self, coords):
        ...
        for i in range(self.hidden_layers + 2):
            if i == 0 :
                ...
                skips = output
            elif i == (self.hidden_layers//2):
                #breakpoint()
                output = torch.cat([output,skips] , -1)    
                output = self.net[i](output)
            # elif i == (self.hidden_layers//2) + 1:    
            #     output = self.net[i](output)
            else:
                output = self.net[i](output)
